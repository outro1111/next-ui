				...
				{items.map((item) => (					
					...
					>
						<div className="flex w-full flex-col gap-1">
							...
							<div className={cn("font-medium", mail.selected === item.id ? "text-foreground" : "text-muted-foreground")}>{item.subject}</div>
						</div>
						<div className={cn("line-clamp-2", mail.selected === item.id ? "text-foreground" : "text-muted-foreground")}>{item.text.substring(0, 300)}</div>
						{item.labels.length ? (
							<div className="flex items-center gap-2">
